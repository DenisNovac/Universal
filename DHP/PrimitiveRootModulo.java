...
// будут почти всегда находиться в байтовом диапазоне
	...
	private static int searchForRoot (int simple, boolean isRandomPlace) {
		...
		// условие необходимо для уменьшения количества генерируемых
		// плохих ключей вроде набора единиц и нулей
		while (true) {
			if (ret<simple && ret!=0) break;
			ret = searchForRoot(simple, true);
		}
